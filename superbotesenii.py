...
third_menu_key.row('Время в Москве', 'Время в Нижнем Тагиле', 'Время в Комсе')
	...
	if len(user) > 0:
		...
		if user['mode'] == 'start':
			...
			if text == 'Время':
				...
				bot.send_message(chat_id, 'Вы попали в подменю времени', reply_markup=third_menu_key)
			...
			if text == 'Назад':
				...
				url = 'https://worldinfotravel.com/ru/country-russia-city-nizhniy-novgorod-time/'
				response = requests.get(url)
				soup = BeautifulSoup(response.text, "html.parser")
				items = soup.find_all('span', class_='time24')
				bot.send_message(chat_id, f'Новости IT сегодня: {items[1].text}')

			elif text == 'Время в Комсе':
				url = 'https://worldinfotravel.com/ru/country-russia-city-nikolayevsk-on-amure-time/'
				response = requests.get(url)
				soup = BeautifulSoup(response.text, "html.parser")
				items = soup.find_all('span', class_='time24')
				bot.send_message(chat_id, f'Время в Комсе: {items[30].text}')

			elif text == 'Время в Нижнем Тагиле':
				url = 'https://worldinfotravel.com/ru/country-russia-city-nizhniy-novgorod-time/'
				response = requests.get(url)
				soup = BeautifulSoup(response.text, "html.parser")
				items = soup.find_all('span', class_='time24')
				bot.send_message(chat_id, f'Время в Нижнем Тагиле: {items[2].text}')
